...
Run summaries also include CPU diagnostics for `GBR`, `VBR`, `SPC`, `SSR`, `FPSCR`, `TRA`, `EXPEVT`, and `INTEVT`, plus TMU channel diagnostics for current counter/constant/control/priority/pending state, plus scheduler diagnostics for synthetic VBlank events, hardware advancement ticks, coalesced hardware batches, max hardware batch size, idle-advance ticks/batches/wake reasons, CPU fast-forwarded instructions/batches, and controller-script state changes. The runner currently uses this batching after SH-4 `sleep` instructions, side-effect-free self-branch waits, and narrow taken backward `bt`/`bf` waits with read-only polling bodies to advance hardware directly to the next timer, enabled VBlank, or controller-script boundary. It also fast-forwards narrow masked `dt`/`bf/s` counted-delay loops, immediate `dt`/`bf` delay loops, known side-effect-free IP.BIN fill/copy/wait loops, the current DOA2 VRAM clear helper loop, DOA2's `0x8C113312-0x8C11331A` system-RAM clear loop, the observed partial `0x8C114254-0x8C114276` init-delay loop, the `0x8C10EDAA-0x8C10EDBC` byte-scan helper, DOA2's no-op callback timeout loop at `0x8C12F99A-0x8C12F9B4`, DOA2's guarded software busy-bit wait at `0x8C130460-0x8C13048E` when its task-helper queues are empty, slot 7 remains the default no-op, and the active work item matches the observed DOA2 task arena stride/state fields; that busy-bit completion also frees the completed task slot so the allocator scan at `0x8C12FD20` can reuse the arena. The runner also fast-forwards forward/backward store-and-`dt` clear loops when those exact loop instructions are outside the active trace filter. The hot IP.BIN copy and DOA2 clear/timeout fast-forwards can take partial batches when the remaining instruction budget is smaller than the remaining loop body.

Current local retail probes for DOA2, Rayman 2, and Legacy of Kain pass the shared IP.BIN splash/reset handoff into the loaded boot binaries. BIOS system function `0` is modeled as a soft-reset handoff to `0x8C010000` with a high-RAM stack and clears boot area mode byte `0x8C0080FE` after the handoff. System function `3` is modeled as the nonterminal check-disc/menu helper used by DOA2's boot path. DOA2 now clears the former `0x8C12ED28/2A` state wait: `0x8C30CA2C` advances from `5` to `6`, then a longer local run reaches IRQ9 acceptance at `0x8C0F8F70` with `VBR=0`. The low BIOS IRQ table now preserves guest writes such as `0x00000224 = 0x8C12BF20`, dispatches IRQ9/IRQB into retail callback handlers instead of faulting at `0x00000600`, passes the accepted event code in banked `r4`, and returns callback-style handlers through a firmware wrapper that restores `SSR/SPC` plus the interrupted `PR` and acknowledges the accepted ASIC source; boot directory/mode bytes `0x8C0080F0..FE` stay seeded for the post-IRQ path while `0x8C0080FE` is cleared on handoff to avoid re-entering IP.BIN. The BIOS boot-directory seed now falls back to root but can derive a media root directory such as `DOA2` from a matching ISO9660 volume/root-directory name, which lets DOA2 load its data directory instead of searching root for `spr_adx.bin`. Loaded media reports GD-ROM disc type `0x80`, reaches successful DMA reads without re-entering IP.BIN, translates raw CUE filesystem FAD reads through the `trackStart + 150` data-area bias, and reports the BIOS GD-ROM send/exec/check lifecycle as processing then completed; a long probe records 17 successful reads, including the DOA2 directory sectors, `sector=432123`, and a 33-sector `sector=412644` asset transfer. The former software busy-bit blocker around `0x8C130460` is now guarded and fast-forwarded when bit 0 of `0x8C2F67FC` is set, callback slot 7 is still the default no-op, DOA2's task-helper queues are empty, and the active queue head is one of the observed task-arena work items below `0x8C2F7640`; synthetic completion also frees the matched slot, clearing the later full-arena allocator scan at `0x8C12FD20`. A 650M run now advances through those waits, records `Boot binary: writes=103289`, and reaches the recurring VBlank IRQ/default-event checkpoint around `0x8C12BF42` with pending source `0x0320`. Focused higher-budget probes show this is not a hard stop: 750M reaches the AICA ARM driver upload/poll around `0x8C104D4C` after 18 successful reads, and 760M reaches 20 successful reads plus the current FIPR/NaN vector-math loop around `0x8C0FAF6C`; SH-4 `FPSCR.FR` now swaps the active FR/XF floating-point register banks. Enriched FPU traces show the first captured `FIPR` at `0x8C0FAF40` already consumes `0xFFC00000` lanes in `fv0`/`fv8`; the parent path at `0x8C0130B6` converts `r14=100` to `fr4=100.0` before calling the helper through `0x8C0FAFC0`, while `0x8C105310 -> 0x8C1056A0` is a command/parameter FIFO writer into `0xA0800400..`, not the source of those FPU lanes. The FPU anomaly log then identifies an earlier repeating NaN path at `0x8C0FB218`, where `fdiv fr3,fr4` executes with both operands at `0.0`; the upstream infinity producer is now traced to `0x8C1038CC`, where a `BF/S` delay-slot `fmul fr4,fr6` repeatedly amplifies `fr6` by about `0x48601040` and stores `+Infinity` to `0x8C2F1740+` through `0x8C1038CE`. SH-4 documentation confirms the not-taken `BF/S` successor is still a delay-slot instruction, so the next retail target is why those generated infinity table entries are later consumed by the `0x8C0FB180..230` vector helper instead of remaining harmless sentinels or being bounded by missing FPU status/exception behavior. The large clear/fill loop around `0x8C12ED90`/`0x8C129E20` is fast-forwarded when trace capture does not cover it. Rayman 2 now reaches `0x8C0DECDA`; Legacy of Kain now reaches `0x8C032164`. Earlier focused traces showed the IP.BIN wait's `[R15+0x0C]` `completion-flag` only shortens the splash delay, because both the flag-set and timeout exits fall through to the BIOS system reset vector.

The latest DOA2 FPU write-log probe shows `fr6` last becomes `+Infinity` at instruction `#58706543` in `0x8C1038CC`; no `fr6` writes appear in the immediate `#58706680..58706770` consumer window. With FPSCR arithmetic status modeled, that first finite overflow records `FPSCR=0x00045015` (`O`/`I` cause plus sticky overflow/inexact flags), while the next infinite-input multiply at `#58706547` clears the per-operation cause bits back to sticky `0x15`. A fast-forward-safe `--fpscr-log` probe through `#58720000` found only those two FPSCR events in the window and no explicit `fpscr` reads/writes. Numbered trace correlation shows the generator exits through `0x8C1038E2` to `0x8C0165B6` at `#58706650`, then the caller runs scalar setup helpers `0x8C100D50`, `0x8C1013B0`, `0x8C1013E0`, `0x8C101410`, `0x8C0F9F30`, and `0x8C0FB900`; the `0x8C0FB900` wrapper at `#58706746..50` only sets `0x8C2E2420 |= 1`. `--fpu-snapshot-log` confirms that the live infinity persists unchanged across the generator return, caller resume, `0x8C0F8DC0`, and `0x8C0F9FA0`: at `#58706765` before `0x8C0F9FAA`, `fv4=[0x47C35000,0x43F00000,0x7F800000,0x431D8100]` and `fv8=[0x42FE0000,0x3F3504F7,0x3E95F612,0x3E800000]`. `--fpu-memory-log` found no immediate stack save/restore path: the generator/consumer window only stores `fr6`/`fr7` into the generated table around `0x8C2F4938..0x8C2F493C`, loads one scalar constant before the vector helper, and stores `fr5` after `fipr`; a wider `fr6` window saw only generated-table stores across `0x8C2F489C..0x8C2F493C`, with no `fr6` loads or `r15` stack transfers. After modeling `FIPR` as always inexact, the consumer appears at `#58950650`: `0x8C0F9FAA` changes `FPSCR` from sticky-only `0x00040015` to `0x00041015`, writes `fr7=0x7F800000`, and the following normal `fdiv` clears the cause field back to `0x00040015`. A decoded FPSCR scan to the consumer filled 4096 events without seeing enabled exception bits, including repeated `sts fpscr,r0` reads around `0x8C1106D2`, so the next target is the generator/table contract or an ABI/live-register boundary rather than a currently enabled FPU trap.
