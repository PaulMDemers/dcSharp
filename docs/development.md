...
- `--fpu-anomaly-log artifacts/logs/fpu.txt --fpu-anomaly-kind nan --fpu-anomaly-limit 512` captures bounded `FR`/`XF` transitions into non-finite single-precision bit patterns after each SH-4 instruction. Use `--fpu-anomaly-kind all|nan|infinity` to include all non-finites, only NaNs, or only infinities. Each line records the instruction count, PC/opcode/trace, changed FPU register, old/new bits, anomaly kind, and `FPSCR`.
