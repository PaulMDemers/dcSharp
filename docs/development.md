...
- `dcsharp_pvr_texture_rgb565.elf`: writes a tiny RGB565 texture into VRAM, emits a textured real-shaped strip with UV corner coordinates, exposes decoded texture state, leaves sampled texture colors in preview pixels, shuts down, and reports `ProgramExit`.
