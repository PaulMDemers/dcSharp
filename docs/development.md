...
The latest DOA2 FPU write-log probe shows `fr6` last becomes `+Infinity` at instruction `#58706543` in `0x8C1038CC`; no `fr6` writes appear in the immediate `#58706680..58706770` consumer window. Numbered trace correlation shows the later `0x8C0FB900` wrapper at `#58706746..50` only sets `0x8C2E2420 |= 1`, then `0x8C0F8DC0` starts at `#58706756` and only loads `fr4=100000.0` before jumping into `0x8C0F9FA0`. The later `0x8C0F9FAA` `fipr fv8,fv4` consumes that live infinity and turns `fr7` into infinity, so the next target is the generator caller/setup before `0x8C1038CC` and whether an earlier call, interrupt, ABI boundary, or missing FPU control/status behavior should have initialized, clobbered, or bounded those live lanes.
