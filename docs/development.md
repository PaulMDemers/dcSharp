...
The latest DOA2 FPU write-log probe shows `fr6` last becomes `+Infinity` at instruction `#58706543` in `0x8C1038CC`; no `fr6` writes appear in the immediate `#58706680..58706770` consumer window. The later `0x8C0F9FAA` `fipr fv8,fv4` consumes that live infinity and turns `fr7` into infinity, so the next target is the caller/setup path around `0x8C0F9FA0` and whether an earlier call, interrupt, ABI boundary, or missing FPU control/status behavior should have initialized, clobbered, or bounded those live lanes.
