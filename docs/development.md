...
The current DOA2 retail 150M probe reaches the TA emit/color-pack frontier with 779,608 TA writes and 32,141 assembled sprite-shaped packets. Those packets are now surfaced in `media boot-smoke`, but their raw sprite coordinate words decode as non-finite values such as `0xFFC00000`, so summaries label them `preview=nonfinite` and the preview renderer skips them rather than painting a misleading single-pixel artifact. The next rendering frontier is to trace why retail is feeding those sentinel coordinates: likely earlier transform/FPU data generation, command grouping, or a missing PVR/TA state bit rather than texture sampling.

PC-tagged TA logs plus PC-filtered memory watches show the TA emit helper reads those sentinel coordinate words from dynamic source buffers such as `0x8C2B6BB0-0x8C2B6BCF` at `0x8C100808-0x8C10082E`. Writes to the same buffers come from the interpolation path around `0x8C100AAA-0x8C100AB2` and vector-scale path around `0x8C1005B2`. A focused trace with the interpolation fast-forward disabled still records `fipr` at `0x8C100A7E/0x8C100A98` consuming `0xFFC00000` lanes and writing the same sentinel values, so the shortcut is not the immediate cause; the next probe should move earlier into the vector/prologue inputs feeding `0x8C100A7A`.
