...
The latest DOA2 FPU write-log probe shows `fr6` last becomes `+Infinity` at instruction `#58706543` in `0x8C1038CC`; no `fr6` writes appear in the immediate `#58706680..58706770` consumer window. With FPSCR arithmetic status modeled, that first finite overflow records `FPSCR=0x00045015` (`O`/`I` cause plus sticky overflow/inexact flags), while the following infinite-input multiplies clear the per-operation cause bits back to sticky `0x15`. Numbered trace correlation shows the generator exits through `0x8C1038E2` to `0x8C0165B6` at `#58706650`, then the caller runs scalar setup helpers `0x8C100D50`, `0x8C1013B0`, `0x8C1013E0`, `0x8C101410`, `0x8C0F9F30`, and `0x8C0FB900`; the `0x8C0FB900` wrapper at `#58706746..50` only sets `0x8C2E2420 |= 1`. `0x8C0F8DC0` then starts at `#58706756` and only loads `fr4=100000.0` before jumping into `0x8C0F9FA0`. The later `0x8C0F9FAA` `fipr fv8,fv4` consumes that live infinity and turns `fr7` into infinity without producing a new finite-overflow cause, so the next target is whether retail code reads sticky FPSCR flags, enables FPU traps, or expects an ABI/interrupt boundary to clobber those live lanes.
