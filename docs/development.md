...
- `--trace-log artifacts/logs/trace.txt --trace-pc 0x8C010000-0x8C010100 --trace-log-limit 4096` writes a bounded filtered SH-4 trace. Repeat `--trace-pc` to capture several disjoint code ranges in one run. Trace capture preserves every instruction that matches the requested PC filter, while CPU fast-forwards can still collapse uncaptured loops outside the filter. SH-4 scalar and vector FPU traces keep their compact normal form for finite results, but append raw source operand bits when `fadd`, `fsub`, `fmul`, `fdiv`, `fmac`, or `fipr` produces `NaN`/`Infinity`; this keeps retail FPU probes diagnosable without expanding ordinary traces.
