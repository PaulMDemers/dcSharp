...
- `tools/probe-retail.ps1` reruns the local ignored retail-disc boot-smoke probes when `retail_discs/` is populated. It checks that DOA2 and Rayman advance beyond the IP.BIN framebuffer clear loop without unmapped stops and that Legacy still reaches the current `0x330441F0` pointer/table blocker.
