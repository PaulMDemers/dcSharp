...
AICA summaries include current named register values plus recent register accesses. Channel summaries decode sample format, sample stride, loop enable, sample address, loop points, pitch, pan/send nibbles, derived left/right balance, volume, key-on state, playback sample/byte counters, and active channel count while remaining silence-safe.
