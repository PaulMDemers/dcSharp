...
- `--fpscr-log artifacts/logs/fpscr.txt --fpscr-instruction 58706480-58720000 --fpscr-limit 512` captures bounded FPSCR changes plus explicit `fpscr` access instructions without disabling CPU fast-forwards through unrelated loops. Each line records the instruction count, PC/opcode/trace, old/new FPSCR value, whether the entry is a status `change` or same-value `access`, and nearest symbol when one is available.
