...
- Use `--memory-write-log <path>` with optional `--memory-write-address <start-end>` and `--memory-write-limit <count>` to capture filtered guest writes after program load and firmware stub installation. This is useful for retail IP.BIN checkpoints where the device log is quiet but a stack/work-area field should change. For example, watching `0x7E000FDC-0x7E000FDF` on the current DOA2 probe captures only the initial `completion-flag` zero write, which confirms the remaining soft-reset timeout is waiting for a flag that is never set rather than losing a later write.
...
Current local retail probes for DOA2, Rayman 2, and Legacy of Kain all reach the same IP.BIN BIOS soft-reset path at `0x8C008524 -> 0x8C0000E8`. The shared checkpoint is a timeout decision: stack word `[R15+0x10]` crosses `0x001E8480` after repeated IP.BIN wait loops, no GD-ROM read commands are issued, and the next compatibility blocker is modeling enough BIOS/GD-ROM boot handoff behavior for IP.BIN to continue into the loaded boot binary instead of requesting reset. The runner now seeds the SH-4 port A data bits IP.BIN copies into AICA configuration and a BIOS work-area language byte of ASCII `1`, so the retail bootstrap takes its language-aware mode `8` path before the remaining timeout. A focused watch of `[R15+0x0C]` (`completion-flag`) shows a single zero initialization write and no setter before soft reset.
