...
- `--fpu-memory-log artifacts/logs/fpu-memory.txt --fpu-memory-register fr6 --fpu-memory-instruction 58700000-58706800 --fpu-memory-limit 512` captures bounded SH-4 FPU memory transfers for `fmov.s`/`fmov.d` load/store forms, including direction, register, effective address, transfer size, memory value, `FPSCR`, instruction count, trace text, and nearest symbol when one is available. Use repeated `--fpu-memory-address <start-end>` filters to narrow the log to stack, work-area, or generated-table ranges.
