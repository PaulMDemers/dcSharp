...
- `tools/probe-retail.ps1` reruns the local ignored retail-disc boot-smoke probes when `retail_discs/` is populated. It checks that DOA2, Rayman, and Legacy advance through IP.BIN framebuffer setup to the current system BIOS soft-reset checkpoint without unmapped stops. With `-LongDoa2`, it also verifies the longer DOA2 path reaches the current loaded-binary string-scan checkpoint after the GD-ROM DMA-read sequence. Legacy verifies the former `0x330441F0` corrupted-IP.BIN pointer/table blocker stays cleared under a longer local retail budget.
