...
- `tools/probe-retail.ps1` reruns the local ignored retail-disc boot-smoke probes when `retail_discs/` is populated. It checks that DOA2 and Rayman advance through the IP.BIN framebuffer setup to the current system BIOS soft-reset checkpoint without unmapped stops and that Legacy still reaches the current `0x330441F0` pointer/table blocker.
