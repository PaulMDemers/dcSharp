...
The current DOA2 retail 150M probe reaches the TA emit/color-pack frontier with 791,728 TA writes and 32,901 assembled sprite-shaped packets. Correct SH-4 `...D` FPU opcode decoding now keeps this phase finite: the probe reports `taSpritePreview=renderable:1470/degenerate:31431/nonfinite:0`, with the same renderer source PCs at header `0x8C1007FA`, control `0x8C10084C`, and payload range `0x8C10084C-0x8C100850`. The preview renderer now paints nonzero VRAM output for the renderable subset, while the remaining degenerate sprites appear to be thin or zero-width UI/text geometry rather than qNaN payloads.

`tools/probe-retail.ps1 -Doa2SpriteProbe` runs the 150M DOA2 JSON checkpoint and asserts the current source groups as `renderable:1470` and `degenerate:31431` at `pc=h:0x8C1007FA/c:0x8C10084C/p:0x8C10084C-0x8C100850`.
