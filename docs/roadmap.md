...
- Inspect retail-adjacent media without booting it. Started: `media inspect` reports raw/CUE/GDI geometry, CUE track layout, IP.BIN-style Dreamcast boot metadata when a readable boot sector is found, and adjacent CUE-directory boot candidates for odd dump layouts.
