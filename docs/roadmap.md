...
- Classify TA command writes before a full renderer. Started with first-word command kind, list type decoding, a real TA parameter decoder skeleton, and a tiny diagnostic opaque strip assembler that consumes fixture-only control/X/Y/color vertex packets.
