...
- Classify TA command writes before a full renderer. Started with first-word command kind, list type decoding, a real TA parameter decoder skeleton, a TA stream control/payload diagnostic view with named polygon/sprite/modifier/user-clip payload slots, real-shaped KOS polygon header/vertex and sprite-header/sprite-geometry fixtures with zero and nonzero mode payloads, a tiny diagnostic opaque strip assembler that consumes fixture-only control/X/Y/color vertex packets, mixed-color Gouraud strip assembly for matching headers, immediate write-order strip/sprite preview composition, and tiny rectangular/skewed sprite quad previews with face-color plus non-twiddled/twiddled RGB565, ARGB4444-alpha, shading, and UV-mode texture paths.
