...
- Add GD-ROM sector read behavior. Started: firmware GD-ROM HLE can read sectors from the loaded media image into system RAM, KOS `cdrom_read_sectors()` PIO/DMA command layouts are wired to the HLE path, queued command ids now report completed/failed status with transferred byte counts, generated local media now includes a tiny nested ISO9660 filesystem, and summaries/fixtures record TOC plus sector/count/destination/status diagnostics for no-media and out-of-range failures, successful raw single-sector, raw multi-sector, missing-file, root directory, nested directory, multi-sector file, and seek/EOF media access.
