...
- Improve the scheduler to coalesce timer advancement. Done for skipped instruction-count calls, SH-4 `sleep` waits, side-effect-free self-branch waits, and masked `dt`/`bf/s` counted-delay loops; broader interruptible idle-loop detection can now build on the same hardware batch path.
