...
- Ratchet local retail probes. Started: `tools/probe-retail.ps1` verifies current local-disc checkpoints, including DOA2, Rayman, and Legacy progressing through IP.BIN framebuffer setup to the current system BIOS soft-reset checkpoint without unmapped stops. Legacy also verifies the former `0x330441F0` corrupted-IP.BIN pointer/table blocker stays cleared under a longer local retail budget. Focused traces now show the shared retail checkpoint is an IP.BIN timeout path that jumps from `0x8C008524` to the BIOS soft-reset vector at `0x8C0000E8` after `[R15+0x10]` crosses `0x001E8480`, before any GD-ROM read commands are issued. Basic BIOS work-area/config defaults now let IP.BIN select its language-aware mode `8` path before that timeout, and a focused watch of `[R15+0x0C]` confirms the `completion-flag` is initialized to zero but never set before reset.
