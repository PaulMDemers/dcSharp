...
- Ratchet local retail probes. Started: `tools/probe-retail.ps1` verifies current local-disc checkpoints, including DOA2 and Rayman progressing through IP.BIN framebuffer setup to the current system BIOS call checkpoint without unmapped stops and Legacy stopping at the current `0x330441F0` pointer/table initialization blocker.
