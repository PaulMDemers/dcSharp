...
- Add fixture expectations for AICA state. Started: manifests can assert current named register values plus decoded channel control, sample, loop, pitch, pan/send, balance, volume, key-on fields, playback sample counters, and playback byte counters.
- Add silence-safe AICA register/channel tracking before audible output. Started: AICA MMIO, sound RAM writes, current named register values, decoded sample format/stride, loop state, key-on state, pan/send balance, touched channel snapshots, and nominal 44.1 kHz PCM16/PCM8 playback position/loop and loop-end diagnostics are captured in audio summaries.
