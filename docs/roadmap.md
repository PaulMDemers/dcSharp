...
- Add GD-ROM sector read behavior. Started: firmware GD-ROM HLE can read sectors from the loaded media image into system RAM, KOS `cdrom_read_sectors()` PIO/DMA command layouts are wired to the HLE path, queued command ids now report completed/failed status with transferred byte counts, generated local media now includes a tiny ISO9660 filesystem, and summaries/fixtures record sector/count/destination/status diagnostics for each read command.
