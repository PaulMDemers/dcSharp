...
- The latest 120M no-trace SA2 profile still reaches `PC=0x8C170AB8` with 47/47 GD-ROM reads and no TA writes, but CPU fast-forward has risen to `116,477,772` and the `0x8C10F440` cache loop plus `0x8C134F3E` record-hash scan are retired from the hot profile. The setup bridge plus recent descriptor/post-setup/tail fast-forwards removed roughly 696K more stepped instructions from the AICA channel/name helper; the next profile-guided targets remain the IP.BIN set-bit/draw path and SA2 clusters around `0x8C134CA8` and `0x8C15B918`, with `0x8C15C57C` now only visible as the fast-forward trigger.
