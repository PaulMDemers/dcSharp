...
1. Trace the WinCE process/object initializer continuation after `0x8C01DD76`; the stable frontier is now an intentionally zero `0x8C1376C0 + 0x1C` scheduler link that propagates to current-thread `+0x48`, followed by an empty slot scan across `0x8C136548-0x8C136584` with `current-wait-delta=7` and `timer-wheel-max-delta=7`.
