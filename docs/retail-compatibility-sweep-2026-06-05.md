...
1. Trace the WinCE scheduler path after the legitimate slot-1 removal at `0x8C017960`; the stable frontier is now an intentionally zero `0x8C1376C0 + 0x1C` scheduler link that propagates to current-thread `+0x48`, a current-thread enqueue/removal pair with no next link, and an empty slot scan across `0x8C136548-0x8C136584` with `current-wait-delta=7` and `timer-wheel-max-delta=7`.
