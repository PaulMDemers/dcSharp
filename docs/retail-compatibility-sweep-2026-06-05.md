...
The priority-bucket side path is now easier to rule in or out from compact logs. A 9.2M priority-object snapshot shows the `0x8C01AAA8` work item at `0x8C137554` with flags `0x00010004`, current-thread `0x8CEEEE9C`, return PC `0x8C018D94`, and resolved object `0x8CEEE448`; that resolved object contains the queued `0x8C137554` pointer in its bucket/list area. `--wince-scheduler-log` now emits this as a work-item summary plus one resolved-object dump. The item is therefore real bookkeeping, but the current scheduler scan still only consults `next-wait-delta`, timer-wheel slots, expired-list state, and `0x8C131AA4/AA8`; none of those are fed by this bucket before the idle loop.
