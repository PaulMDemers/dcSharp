...
1. Trace the real writer or state machine that should populate Sega Rally's `0x8C131AA4/AA8` dispatch ring and `0x8C1319A0` priority masks; the controlled `+0x1C`/region/ring pokes expose later gates but perturb scheduler/module state and are not fixes.
