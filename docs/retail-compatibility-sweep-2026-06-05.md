...
1. Trace the WinCE runqueue head set/clear path around `0x8C017790-0x8C0178B4`; the stable frontier is now an intentionally zero `0x8C1376C0 + 0x1C` scheduler link that propagates to current-thread `+0x48`, a current-thread enqueue/removal pair with no next link, and a transient `runqueue-or-thread-list-head=0x19` that is cleared before the loop returns to dispatch.
