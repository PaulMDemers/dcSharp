...
1. Trace the empty WinCE scheduler list rooted near `0x8C1365AC`; the former `0x8C017790-0x8C0178B4` runqueue-head suspicion is now explained as `kernel-tick-delta=0x19` being moved through `scheduler-pending-tick-delta` at `0x8C1364F4`, then drained because the list is empty.
