...
A changed-only current-thread/object write trace confirms the final `0x0211` current-thread flags are guest-driven rather than a stale queue-removal bug: initialization writes `0x0011`, timer-wheel enqueue at `0x8C0165D0` raises it to `0x0111`, scheduler removal at `0x8C0179A6` lowers it back to `0x0011`, and `0x8C01AB18` raises it to `0x0211` after the priority-bucket insert. The mapped module caller at `0x01E324DC-0x01E32594` directly tests the return from `CURPROC.SetProcPermissions`; the stateful HLE returns the old mask, exposes the real `WIN32.CreateCrit` initialization path, and records stable handles for critical sections at `0x01E4C0C0`, `0x01E4C014`, and `0x01E4C060`. The parent then reads `current-thread +0x48` at `0x8CEEEEE4`; because that field remains zero it exits toward `0x8C023690` and the scheduler. A 20M post-HLE probe still reaches the same scheduler/tick loop with no GD-ROM reads or TA work.
...
A no-poke producer trace narrows that descriptor diagnosis further: `0x8C01DD56-0x8C01DD58` computes `source +0x0C` as `(source byte 0 + 1) << 25`, so `0x02000000` is a deliberate guest descriptor value rather than a host translation artifact. The same initializer writes literal `0x8C011924` to `source +0x20`, hard-zeroes `source +0x1C`, and later computes `source +0x38 = source +0x0C + source +0x18 = 0x8E010000`. `--wince-scheduler-log` now reports this descriptor summary directly, including the region mismatch, derived-base match, and null copy-source field. It also reports the active object chain rooted at `0x8C1364B8`; a 20M compact probe shows that chain as a single coherent `0x8CEEE6FC -> null` node, so the current Sega Rally blocker is still the missing scheduler dispatch/wake producer rather than runaway list construction.

The post-scheduler return path at `0x8C0123C2-0x8C012456` has also been decoded. When the scheduler returns the current thread, it indexes the `0x8C1318A0` dispatch table using `source +0x0C >> 23`, selects `0x8C135B40` for the current `0x02000000` descriptor, restores the thread context, and returns through `SPC=0x8C018C90`. The repeated idle path is the zero-return branch through `0x8C01246E -> 0x8C0120FC -> 0x8C0123AE`. The compact scheduler log now captures the first dispatch-table slots: the latest 20M no-watch probe records slots 0/1 as `0x8C135B40`, slots 2/3 as `0x8C012B2C`, while dispatch-ring head/tail `0x8C131AA4/AA8` and all timer-wheel slots remain zero. A 50M distinct-read follow-up confirms the priority bucket is not consumed after insertion: queued `0x8CEEE450=0x8C137554` is never read after `0x8C016398`, while the hot loop reads `0x8C131AA4/AA8` as zero at `0x8C0176F2/0x8C0176F4` and no watched write makes either dispatch slot nonzero.

A 14M restored-thread trace through `0x8C018C90` shows the first resumed path calling `0x8C01E1D8`, using the current object `0x8CEEE6FC`, storing the selected region base `0x01CC0000` at `0x8C131B28`, then resolving a module/list root at `0x8CEEE5F4` and probing several API names through `0x8C0238E0`; the observed lookups still return zero in this window. The scheduler helper at `0x8C0127B6` is now split by the byte at `0x8C131885`: before the restored path it is zero and the helper takes the slow/busy branch, while later calls see it as one, clear it at `0x8C0127CA`, call the target, and usually restore it at `0x8C0127EC`. The `0x8C01AAA8` call is the exception of interest: it sets current-thread flags to `0x0211`, stores the per-object bucket pointer at current-thread `+0x34`, returns `0x102`, sees `0x8C131884 == 1`, branches through `0x8C0127F6`, and writes `0x102` into the current-thread saved context without restoring `0x8C131885`. A 16M follow-up labels that saved context as current-thread `+0x70=0x102`, `+0xAC=0x8C137544`, `+0xB0=0x8C02647A`, and `+0xB4=0x40008000`; the final snapshot still has `current-thread +0x48=0`, `0x8C131AA4/AA8=0`, all timer-wheel slots empty, and no GD-ROM/TA work. The compact scheduler log now splits the `0x8C131884` word into byte fields and labels the wrapper context saves so later probes can catch this yield-request/critical-byte state without memory-write watches.

A narrow re-entry trace of `#9081078-#9081300` shows the post-wrapper control flow is not yet resuming saved PR `0x8C02647A`. After the wrapper saves `0x102` and the SP/PR/SR context, it branches to `0x8C012398`, clears `current-thread +0x5C` and the volatile saved-register slots, enters `0x8C0123AE`, and clears both `0x8C131884`/`0x8C131885` with the word store at `0x8C0123C0`. The scheduler scan sets wait-active, calls the `0x8C0176A8` pending-tick helper, reads `next-wait-delta=0`, checks timer-wheel slot 7 and finds it empty, writes the updated tail/wait bookkeeping, returns `r0=0`, and immediately takes the idle branch through `0x8C01246E -> 0x8C0120FC -> 0x8C0123AE`. The practical frontier is now the missing producer for `0x8C136544` or the timer-wheel/runnable dispatch fields rather than the wrapper return save itself.

The priority-bucket side path is now easier to rule in or out from compact logs. A 9.2M priority-object snapshot shows the `0x8C01AAA8` work item at `0x8C137554` with flags `0x00010004`, current-thread `0x8CEEEE9C`, return PC `0x8C018D94`, and resolved object `0x8CEEE448`; that resolved object contains the queued `0x8C137554` pointer in its bucket/list area. `--wince-scheduler-log` now emits this as a work-item summary plus one resolved-object dump. A 12M read/write watch shows the bucket is only read during setup/insertion (`0x8C0150CE` and `0x8C01636E`) in that window; after insertion, the hot scheduler loop does not read the bucket. The same watch confirms `0x8C136544` briefly receives the current-thread object from the timer-wheel scan at `0x8C0179CE`, then `0x8C01AB1E` clears it immediately after the priority-bucket insert. The item is therefore real bookkeeping, but the current scheduler scan still only consults `next-wait-delta`, timer-wheel slots, expired-list state, and the dispatch ring; none of those are fed by this bucket before the idle loop. A synthetic ring poke decodes the ring path as head/tail `0x8C131AA4/AA8`, priority bytes at `0x8C131AAC + head`, priority masks at `0x8C1319A0 + priority*4`, and a final selected queue at `0x8C131B40`; forcing those fields still falls back to idle if the selected queue is empty. An 80M exact producer sweep over the ring, priority masks, selected queue, bucket, and work item records no real writes to ring head/tail, priority bytes, or priority masks; only setup writes touch `0x8C131AA0` and selected-queue state `0x8C131B44`. The scheduler log now emits one-line `dispatch-ring-summary` and `yield-handoff-summary` rows so future probes show the empty ring, masks, priorities, selected queue, current-thread flags, shaped work item, resolved object, and saved wrapper context together. A 12M handoff snapshot records `thread-flags=0x00000211`, `work-item=0x8C137554`, `resolved-object=0x8CEEE448`, `saved-return=0x00000102`, and `saved-pr=0x8C02647A`; a nearby `0x8C012730-0x8C012780` trace shows the recurring `0x8C012768` path only ORs the yield byte back in and `rte`s to the interrupted scheduler PC. The next target is the missing activation/dispatch consumer for the `0x8CEEE448` bucket.
...
A producer/consumer trace splits that state from the actual missing queue source. `0x8C131AA0=0x8C012296` is installed once during cached kernel table setup at `0xAC013456`; the steady scheduler loop instead repeatedly reads dispatch-ring head/tail `0x8C131AA4` and `0x8C131AA8`, both still zero. `0x8C131B20=0x8C136410` is initialized as part of a larger module/region record at `0x8C131B00` by `0x8C0295D0-0x8C0295F2`. A real no-poke consumer trace shows the module/list root is not globally missing: `0x8C022F4C` writes `0x8C131B24=0x8CEEE5F4`, and `0x8C01E5FA` later walks that root through the live module object containing the mapped entrypoint metadata. The corrected frontier is therefore the real producer/meaning of the dispatch ring, plus the reason the synthetic region-matched path reaches a later loop with the module root absent, not the setup-only vector entry at `0x8C131AA0`.
...
1. Trace the real activation path that should bridge Sega Rally's `0x8CEEE448` priority bucket into `0x8C131AA4/AA8`, `0x8C131AAC..AAF`, `0x8C1319A0..AC`, or the selected queue at `0x8C131B40`; the decoded yield-byte maintenance path at `0x8C012768` and the controlled `+0x1C`/region/ring pokes expose later gates but are not fixes.
