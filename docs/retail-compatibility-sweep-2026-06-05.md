...
1. Trace the producer for `scheduler-expired-list-head` at `0x8C1365AC`; the consumer is now decoded, and the live blocker is that no object is inserted there after the current-thread timer-wheel enqueue/removal pair.
