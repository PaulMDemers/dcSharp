...
## AICA Mirror Copy Function

The AICA mirror-copy accelerator now covers the function entry at `0x8C135DDC`, folding setup plus all 24 mirrored entries and returning directly through `PR`. The older branch-tail helper remains in place for paths that enter mid-loop.

The 50M probe `sa2-aica-mirror-function-50m-20260612-160925` stayed at `PC=0x8C15C7F4`, kept `PVR registers=2255`, and still had no GD-ROM reads. CPU fast-forward rose from `49,091,870` to `49,129,876`, while CPU fast-forward batches stayed flat at `39,149`; the mirror-copy body `0x8C135DDE-0x8C135E1C` dropped out of the top profile, leaving only `0x8C135DDC` as the function-entry trigger.
