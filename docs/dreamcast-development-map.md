...
- Scan retail AICA RAM for queue-like structures and uploaded ARM driver markers, then report likely queue candidates. Queue snapshots now classify fixed and data-pointer-matched command/response queues and filter non-fixed candidates through forward data pointers plus pending packet-size sanity checks. AICA RAM region diagnostics now identify uploaded ARM/control/sample regions; current SA2 120M evidence shows an uploaded AICA program but no KOS-style mailbox queue, so keep extending this with title-specific ARM-driver markers as they appear.
