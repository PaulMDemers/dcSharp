...
public sealed class DreamcastRunner
{
    public DreamcastRunResult Run(ElfFile elf, DreamcastRunOptions options)
    {
        ...
        try
        {
            ...
            return DreamcastRunResult.InstructionLimit(load, cpu.State, traceTail.ToArray(), traceLog.ToArray(), memory.DeviceAccesses.ToArray(), memory.SerialOutput.ToArray(), memory.CreateVideoSnapshot(), memory.CreateAudioSnapshot());
        }
        catch (UnsupportedInstructionException ex)
        {
            ...
            {
                return DreamcastRunResult.ProgramExit(load, cpu.State, traceTail.ToArray(), traceLog.ToArray(), memory.DeviceAccesses.ToArray(), serialOutput, memory.CreateVideoSnapshot(), memory.CreateAudioSnapshot(), ex.Pc, ex.Opcode, ex.Message);
            }

            return DreamcastRunResult.UnsupportedInstruction(load, cpu.State, traceTail.ToArray(), traceLog.ToArray(), memory.DeviceAccesses.ToArray(), memory.SerialOutput.ToArray(), memory.CreateVideoSnapshot(), memory.CreateAudioSnapshot(), ex.Pc, ex.Opcode, ex.Message);
        }
        catch (MemoryMapException ex)
        {
            return DreamcastRunResult.MemoryFault(load, cpu.State, traceTail.ToArray(), traceLog.ToArray(), memory.DeviceAccesses.ToArray(), memory.SerialOutput.ToArray(), memory.CreateVideoSnapshot(), memory.CreateAudioSnapshot(), ex.Message);
        }
        catch (DreamcastFirmwareExitException ex)
        {
            return DreamcastRunResult.FirmwareExit(load, cpu.State, traceTail.ToArray(), traceLog.ToArray(), memory.DeviceAccesses.ToArray(), memory.SerialOutput.ToArray(), memory.CreateVideoSnapshot(), memory.CreateAudioSnapshot(), ex.Message);
        }
    }
    ...
    ushort? StopOpcode)
{
    public static DreamcastRunResult InstructionLimit(
        ...
        DreamcastVideoSnapshot video,
        DreamcastAudioSnapshot audio) =>
        new(load, Sh4StateSnapshot.From(state), traceTail, traceLog, deviceAccesses, serialOutput, video, audio, DreamcastStopReason.InstructionLimit, "Instruction limit reached", null, null);
    ...
    public static DreamcastRunResult UnsupportedInstruction(
        ...
        ushort opcode,
        string detail) =>
        new(load, Sh4StateSnapshot.From(state), traceTail, traceLog, deviceAccesses, serialOutput, video, audio, DreamcastStopReason.UnsupportedInstruction, detail, pc, opcode);
    ...
    public static DreamcastRunResult ProgramExit(
        ...
        ushort opcode,
        string detail) =>
        new(load, Sh4StateSnapshot.From(state), traceTail, traceLog, deviceAccesses, serialOutput, video, audio, DreamcastStopReason.ProgramExit, $"Program returned after KOS shutdown at 0x{pc:X8}: {detail}", pc, opcode);
    ...
    public static DreamcastRunResult MemoryFault(
        ...
        DreamcastAudioSnapshot audio,
        string detail) =>
        new(load, Sh4StateSnapshot.From(state), traceTail, traceLog, deviceAccesses, serialOutput, video, audio, DreamcastStopReason.MemoryFault, detail, state.Pc, null);
    ...
    public static DreamcastRunResult FirmwareExit(
        ...
        DreamcastAudioSnapshot audio,
        string detail) =>
        new(load, Sh4StateSnapshot.From(state), traceTail, traceLog, deviceAccesses, serialOutput, video, audio, DreamcastStopReason.FirmwareExit, detail, state.Pc, null);
}
