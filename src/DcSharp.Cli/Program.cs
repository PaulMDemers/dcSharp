...
static void PrintAudioActivity(DreamcastAudioSummary audio)
{
    ...
    {
        Console.WriteLine($"  AICA queue {queue.Role}: offset={queue.OffsetHex}, data={queue.DataHex}, size={queue.SizeHex}, head={queue.HeadHex}, tail={queue.TailHex}, valid={queue.Valid}, processOk={queue.ProcessOk}, pending={queue.Pending}");
    }
    ...
    foreach (var field in audio.DriverFields)
    {
        Console.WriteLine($"  AICA driver {field.Name}: offset={field.OffsetHex}, addr={field.AddressHex}, value={field.ValueHex}, reads={field.ReadCount}, writes={field.WriteCount}, lastReadPc={field.LastReadPcHex ?? "none"}, lastWritePc={field.LastWritePcHex ?? "none"}, area={field.Area}");
    }
...
static void RunElf(string path, string[] args)
{
    ...
    foreach (var marker in result.Audio.TextMarkers.TakeLast(8))
    {
        Console.WriteLine($"  AICA text: offset={marker.OffsetHex}, length={marker.Length}, text=\"{marker.Text}\"");
    }
    ...
    foreach (var field in result.Audio.DriverFields)
    {
        Console.WriteLine($"  AICA driver {field.Name}: offset={field.OffsetHex}, addr={field.AddressHex}, value={field.ValueHex}, reads={field.ReadCount}, writes={field.WriteCount}, lastReadPc={field.LastReadPcHex ?? "none"}, lastWritePc={field.LastWritePcHex ?? "none"}, area={field.Area}");
    }
...
[
    ...
    new(0x8C13_1B1Cu, "module-record-owner-list"),
    new(0x8C13_1B20u, "module-or-file-list-link"),
    new(0x8C13_1B24u, "module-or-file-list-root"),
    new(0x8C13_1B28u, "module-or-file-region-base"),
...
static CliRunOptions ParseRunOptions(string[] args)
{
    ...
    string? stopOnPvrTaControlKind = null;
    string? stopOnPvrTaControlList = null;
    ...
    for (var index = 0; index < args.Length; index++)
    {
        switch (args[index])
        {
            ...
            case "--memory-read-limit" when index + 1 < args.Length && int.TryParse(args[index + 1], out var parsedMemoryReadLimit):
                ...
                index++;
                break;
            case "--memory-read-distinct":
                memoryReadDistinct = true;
                break;
            ...
            case "--stop-on-device-domain" when index + 1 < args.Length:
                ...
                index++;
                break;
            case "--stop-on-pvr-ta-write":
                stopOnPvrTaWrite = true;
                break;
            case "--stop-after-pvr-ta-writes" when index + 1 < args.Length && int.TryParse(args[index + 1], out var parsedStopAfterPvrTaWrites):
                stopAfterPvrTaWrites = parsedStopAfterPvrTaWrites;
                index++;
                break;
            case "--stop-on-pvr-ta-control-kind" when index + 1 < args.Length:
                stopOnPvrTaControlKind = args[++index];
                break;
            case "--stop-on-pvr-ta-control-list" when index + 1 < args.Length:
                stopOnPvrTaControlList = args[++index];
                break;
            case "--stop-after-gdrom-reads" when index + 1 < args.Length && int.TryParse(args[index + 1], out var parsedStopAfterGdromReads):
    ...
    if (stopAfterPvrTaWrites is <= 0)
    {
        throw new InvalidDataException("--stop-after-pvr-ta-writes must be greater than zero.");
    }
    ...
    return new CliRunOptions(
        new DreamcastRunOptions(
            ...
            StopAfterPvrTaWrites: stopAfterPvrTaWrites,
            PvrTaControlStop: stopOnPvrTaControlKind is null && stopOnPvrTaControlList is null
                ? null
                : new DreamcastPvrTaControlStop(stopOnPvrTaControlKind, stopOnPvrTaControlList),
