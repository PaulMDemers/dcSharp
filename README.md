...
- KallistiOS samples:
  ...
  - `samples/kos/timer_irq_masked`: bare-metal SH fixture that leaves TMU0 pending while SH-4 interrupts are masked.
  - `samples/kos/timer_irq_accept`: bare-metal SH fixture that accepts a TMU0 interrupt, clears it in the handler, and returns through `rte`.
...
wsl -e bash tools/kos/build-sample.sh samples/kos/timer_irq_masked
dotnet run --project src/DcSharp.Cli -- run artifacts/kos/dcsharp_timer_irq_masked.elf --instructions 1000000 --trace-tail 24
wsl -e bash tools/kos/build-sample.sh samples/kos/timer_irq_accept
dotnet run --project src/DcSharp.Cli -- run artifacts/kos/dcsharp_timer_irq_accept.elf --instructions 1000000 --trace-tail 24
...
```bash
dotnet run --project src/DcSharp.Cli -c Release -- fixtures fixtures/kos.json --validate-only
dotnet run --project src/DcSharp.Cli -c Release -- fixtures fixtures/kos.json
dotnet run --project src/DcSharp.Cli -c Release -- fixtures fixtures/kos.json --filter input_idle
dotnet run --project src/DcSharp.Cli -c Release -- fixtures fixtures/kos.json --report-json artifacts/reports/kos-fixtures.json
```
...
- Low-RAM firmware vectors include return-zero sysinfo/flash stubs, a GD-ROM HLE dispatcher, boot directory/mode work-area bytes for retail IP.BIN handoff checks, a writable BIOS interrupt handler table for VBR-zero retail IRQ dispatch, and a non-returning system BIOS exit trap.
...
- SH-4 store queue writes are buffered and flushed by `pref @Rn` through QACR0/QACR1; zero-QACR retail flushes currently fall back to the TA input aperture to stand in for MMU-backed SQ mappings used by DOA2.
