...
- Current AICA register values, register writes, sound RAM changes, decoded channel state, compressed/streamed sample metadata, pan/send balance diagnostics, and silence-safe PCM playback sample/byte counters are captured and can be asserted by fixtures without producing host audio yet.
...
- Extend silence-safe AICA playback timing into ADPCM decode/position stepping and optional WAV-dump diagnostics.
