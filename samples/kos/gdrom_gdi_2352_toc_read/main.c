...
#define DCSHARP_GDROM_TRACK 5
#define DCSHARP_GDROM_DATA_FAD 45200
#define DCSHARP_GDROM_LEADOUT 45220
...
int main(int argc, char **argv) {
    ...
    printf("dcSharp GD-ROM 2352 toc=%d read=%d first=%lu last=%lu located=%lu leadout=%lu sentinel=%02x%02x%02x%02x\n",
           toc_status,
           read_status,
    ...
    if(toc_status != ERR_OK || read_status != ERR_OK) {
        printf("dcSharp GD-ROM 2352 read failed\n");
        return 1;
    }
    ...
    if(TOC_TRACK(toc.first) != DCSHARP_GDROM_TRACK ||
       ...
       TOC_LBA(toc.leadout_sector) != DCSHARP_GDROM_LEADOUT) {
        printf("dcSharp GD-ROM 2352 TOC mismatch\n");
        return 2;
    }

    if(memcmp(sector_buffer, "M235", 4) != 0) {
        printf("dcSharp GD-ROM 2352 sentinel mismatch\n");
        return 3;
    }

    printf("dcSharp GD-ROM 2352 probe done\n");
    return 0;
}
