...
int main(int argc, char **argv) {
    ...
    const uint32_t texture_base = 0x0000d000u;
    ...
    DCSHARP_PVR_VRAM16(texture_base + (((7u * 8u) + 0u) * 2u)) = 0x001fu;
    DCSHARP_PVR_VRAM16(texture_base + (((4u * 8u) + 4u) * 2u)) = 0xffffu;

    write_words(header, 8);
    write_words(vertices, 16);

    printf("dcSharp PVR skewed sprite texture RGB565 probe: texture_base=0x%08lx mode3=0x%08lx auv=0x%08lx cuv=0x%08lx ta_init=0x%08lx\n",
