...
int main(int argc, char **argv) {
    ...
    for(uint32_t index = 0; index < 64; index++) {
        AICA_RAM8(index) = (uint8_t)(0x40u + index);
    }

    AICA_REG(0x0004) = 0x00000000u;
    AICA_REG(0x0008) = 0x00000000u;
    ...
    AICA_REG(0x0000) = 0x0000c080u;

    printf("dcSharp AICA PCM8 playback probe start ctrl=0x%08lx loop_end=%lu\n",
           (unsigned long)AICA_REG(0x0000),
           (unsigned long)AICA_REG(0x000c));

    thd_sleep(20);

    printf("dcSharp AICA PCM8 playback probe done\n");
    return 0;
}
