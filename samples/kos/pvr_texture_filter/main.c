...
int main(int argc, char **argv) {
    ...
    const uint32_t texture_base = 0x00008000u;
    const uint32_t mode2 = 0x0001a000u;
    const uint32_t mode3 = 0x0c008000u;
    ...
    DCSHARP_PVR_VRAM16(texture_base + (((3u * 8u) + 3u) * 2u)) = 0xf800u;
    DCSHARP_PVR_VRAM16(texture_base + (((3u * 8u) + 4u) * 2u)) = 0x07e0u;
    DCSHARP_PVR_VRAM16(texture_base + (((4u * 8u) + 3u) * 2u)) = 0x001fu;
    DCSHARP_PVR_VRAM16(texture_base + (((4u * 8u) + 4u) * 2u)) = 0xffffu;

    write_words(header, 8);
    write_pvr_vertex(0xe0000000u, 0x3f800000u, 0x3f800000u, 0x00000000u, 0x00000000u);
    write_pvr_vertex(0xe0000000u, 0x40400000u, 0x3f800000u, 0x3f800000u, 0x00000000u);
    write_pvr_vertex(0xf0000000u, 0x3f800000u, 0x40400000u, 0x00000000u, 0x3f800000u);

    printf("dcSharp PVR texture filter probe: texture_base=0x%08lx mode2=0x%08lx mode3=0x%08lx ta_init=0x%08lx\n",
