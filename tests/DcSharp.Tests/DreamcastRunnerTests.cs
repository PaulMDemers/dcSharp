...
public class DreamcastRunnerTests
{
    ...
    [Fact]
    public void CapturesFpuRegisterWriteLogForSelectedRegisters()
    {
        var raw = new byte[4];
        raw[0] = 0x53; // fdiv fr5,fr4
        raw[1] = 0xF4;
        raw[2] = 0x4C; // fmov.s fr4,fr6
        raw[3] = 0xF6;
        ...
        Assert.Collection(
            ...
            write =>
            {
                ...
                Assert.Equal("fr6", write.Register);
                Assert.Contains("fmov.s fr4,fr6", write.Trace, StringComparison.Ordinal);
            });
    }
    ...
    [Fact]
    public void CapturesFpuMemoryTransferLogForSelectedRegisters()
    {
        var raw = new byte[4];
        ...
        Assert.Collection(
            result.FpuMemoryTransfers,
            transfer =>
            {
                Assert.Equal(1UL, transfer.Instruction);
                Assert.Equal("fr4", transfer.Register);
                Assert.Equal(0x8C01_00FCu, transfer.Address);
            },
            transfer =>
            {
    ...
    [Fact]
    public void RunCanStopOnPvrTaCommandWrite()
    {
        var result = new DreamcastRunner().RunRawBinary(
            CreatePvrTaWriteBootBinary(),
            new DreamcastRunOptions(
                InstructionLimit: 10,
                TraceTailLength: 4,
        ...
        Assert.Contains("PVR TA command write count reached 1", result.StopDetail);
    ...
    public void StructuredRunSummaryIncludesNearestSymbols()
    {
        ...
        var result = new DreamcastRunResult(
            ...
            new DreamcastVideoSnapshot(0, 0, 0, "0x00000000", null, null, [], [], [], [], [], [], [], [], new(0, 0, 0), DreamcastPvrPreviewRenderStats.Empty, []),
