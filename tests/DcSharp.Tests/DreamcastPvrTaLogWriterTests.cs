...
public class DreamcastPvrTaLogWriterTests
{
    ...
    public void WritesDecodedTailWithOriginalStreamIndexes()
    {
        ...
        Assert.DoesNotContain("#0 addr=0x10000000", text);
        Assert.Contains("#2 addr=0x10000008 region=TA_INPUT role=Payload controlKind=PolygonHeader rawKind=ModifierVolume list=OpaquePolygon endOfStrip=False payloadIndex=1 payloadRemaining=5 payloadName=Mode2 controlValue=0x80840000 value=0x80100000", text);
        Assert.Contains("#3 addr=0x1000000C region=TA_INPUT role=Payload controlKind=PolygonHeader rawKind=Unknown list=OpaquePolygon endOfStrip=False payloadIndex=2 payloadRemaining=4 payloadName=Mode3 controlValue=0x80840000 value=0x48001234", text);
    }
